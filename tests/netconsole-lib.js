...
export function categorizeLine(line) {
...
export function classifyLines(linesA, linesB) {
...
const CISCO_KW = ['cisco','catalyst','nexus','interface gigabitethernet','router bgp','ip route','ip ssh','aaa new-model','spanning-tree vlan','switchport','ip access-list','bfd-template','ip sla','snmp-server','logging ','ntp server','service password','enable secret','username ','line vty','ip http','crypto key'];
const HUAWEI_KW = ['huawei','vrp','interface vlanif','display ','undo ','sysname','stelnet','hwtacacs','local-user','authentication-mode','speed auto','duplex auto','dhcp enable','vlan batch','port link-type','port default vlan','voice-vlan','ntp-service','snmp-agent','info-center','radius-server','acl number','traffic classifier','traffic behavior'];

export function detectVendor(config, lowerConfig) {
...
const SWITCH_KW = ['switchport','spanning-tree','vlan','voice vlan','port-security','poe','lldp run','vtp','mac address-table','storm-control','portfast','bpduguard','stack','stackwise','port link-type','port default vlan'];
const ROUTER_KW = ['router bgp','router ospf','router eigrp','router isis','ip route 0.0.0.0','bfd','ip sla','nat','redistribute','route-map','neighbor ','vrf','ip vrf','mpls','l2vpn','xconnect'];

export function detectDeviceType(config, lowerConfig) {
  var c = lowerConfig || (config || '').toLowerCase();
...
export function hasFeature(config, keywords, lowerConfig) {
  var c = lowerConfig || (config || '').toLowerCase();
...
export const AGENT_TO_SKILL_CATEGORY = {
...
export function getNetworkingSkills(vendor, deviceType, agentIds, configText) {
  var skills = [];
  var seen = {};
...
export const AGENT_MAP = Object.fromEntries(AGENTS.map(function(a) { return [a.id, a]; }));
